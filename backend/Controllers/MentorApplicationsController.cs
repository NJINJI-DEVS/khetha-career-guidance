...
public class MentorApplicationsController : ControllerBase
{
    ...
    [HttpGet]
    [Authorize(Policy = "AdminOnly")]
    public async Task<ActionResult<List<MentorApplication>>> GetAll(CancellationToken ct) =>
        Ok(await _db.MentorApplications.AsNoTracking().OrderByDescending(a => a.SubmittedAt).ToListAsync(ct));
